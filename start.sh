...
show_help() {
  ...
  echo
  echo "Usage: start.sh TEST CONFIG [OPTION]..."
  echo
...
check_arguments() {
  ...
  # remove '/' at the end of the path if it present
  if [ "${SDL_CORE: -1}" = "/" ]; then
...
copy_logs() {
  local REPORT_DIR_PTRNS=("SDLLogs*" "ATFLogs*" "TestingReports_*")
...
run() {
  ...
  local OPTIONS="--report-path=${REPORT_PATH} $OPTIONS"
  dbg "OPTIONS: "$OPTIONS
...
set_config() {
  if [ ! -z $CONFIG ]; then
    ...
    cp modules/config.lua modules/config.lua.BAK
    cp modules/config.$CONFIG.lua modules/config.lua
  else
...
restore_config() {
  if [ ! -z $CONFIG ]; then
    cp modules/config.lua.BAK modules/config.lua
    rm modules/config.lua.BAK
  fi
}
